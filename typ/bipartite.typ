...
#let bipartite-theme() = data => {
  ...
  let plain(slide-info, bodies) = {
    if bodies.len() != 1 {
      panic("plain variant of bipartite theme only supports one body per slide")
    }
    let body = bodies.first()
  ...
  let full(slide-info, bodies) = {
    if bodies.len() != 1 {
      panic("full dark variant of bipartite theme only supports one body per slide")
    }
    let body = bodies.first()
